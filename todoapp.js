...
function deleteTodo(e) {
  // arayüzden todo siler
  if (e.target.className === "fa fa-remove") {
    // ul üzerinde tıklanan yerin clası bu ise.
...
function loadAllTodosToUI() {
  // sayfa yüklendiğinde çalışan fonksiyon  storage daki todo ları yükler.

  let todos = getTodosFromStorage(); // storage daki todo ları alan fonksiyon.
  todos.forEach((todo) => {
    // herbir todo yu arayüze ekler fonksiyon ile.
    addTodoToUI(todo);
  });
}
...
function addTodoToUI(newTodo) {
  // String değeri list item olarak arayüze ekler.
  // List item oluştu.
...
function getTodosFromStorage() {
  // storage de array varsa alır yoksa oluşturur.

  let todos;

  if (localStorage.getItem("todos") === null) {
    // Storage da todos keyi yoksa null bir obje döner.
    todos = [];
  } else {
...
function showAlert(type, massage) {
  //bootstrap ile arayüzde mesaj gösterir.
  ...
  setTimeout(function () {
    // gelen uyarıyı 1,5 saniye sonra siler
    alert.remove();
  }, 1500);
}
